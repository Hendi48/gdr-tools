 ...
 *
 * Will use only Hitachi debug commands
 * for that task.
 *
 ...
 * date: 7th March 2006
 * platform: linux
 * modified by: Arne Brutschy, Florian Doerfler
 *
 */
...
int hex_atoi(void *dest, char *src, int dest_len)
{
	...
	for (i = src_last; i >= 0; i--) {
		if (src[i] >= '0' && src[i] <= '9')
			...
			return EXIT_FAILURE; // invalid hex digit
...
int main(int argc, char *argv[])
{
	...
	if (argc != 5) {
		printf("usage: debug_flashsec device firmware_image target_sector_address sector_size\n");
		return EXIT_FAILURE;
	}
	...
	// no round-up or checksum needed as we're using debug commands
	buffer_len = (sizeof(control_code) + sizeof(erase_code) + sizeof(program_code) + sector_size);
	...
	// convert sector address to little endian if needed
	...
	// adjust sector address in control code
	...
	*((unsigned int *)&control_code[48]) = i; // little endian sector_size
	...
	// copy data from firmware image to buffer
	...
	if (ioctl(fd, CDROM_SEND_PACKET, &cgc) == -1) {
		...
		float onepercent = buffer_len / 100.0;
		int percent = 0;
		printf("uploading code 0%%");
		...
		for (i = 0; i < buffer_len; i++) {
			...
			cgc.cmd[3] = 0x54;
			cgc.cmd[4] = 0xCC;
			cgc.cmd[5] = (unsigned char)buffer[i];
